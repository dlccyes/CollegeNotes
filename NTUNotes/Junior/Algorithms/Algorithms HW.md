...
而小明雖然熱愛接電路，但一個人的熱情是有限度的。每過多花一絲精力在 debug 上面，小明對電路的熱情就會削減一分，若花了一定程度的精力都還沒有 debug 成功，小明的熱情就會被消磨殆盡，從此不再願意碰任何電路，自然也不會再繼續 debug。因此，小明能夠做的，便是在心態炸裂之前，做 best effort debugging，盡可能 de 最可能出問題的元件們，提高 debug 成功率，阿 de 不出來 ... 就算了。
...
- `P[i,mP]` 代表，給定 `MP=mP`， element 1 to i 之中的  subset with max sum of 出問題可能性的 sum of 出問題可能性的
- `E[i,mP]` 代表，給定 `MP=mP`， element 1 to i 之中的  subset with max sum of 出問題可能性的 elements

令 n = 可 debug 元件數目；a = 精力 surplus 的 weight
...
最後 return 的第一項就是預期 debug 成功可能性。再次強調這個數值並非真的機率。小明並不是神仙，無法判斷出實際的機率，但這個數字僅具有相對意義：數字愈大，debug 愈可能成功。
注意不同 a 之下，得到的最終數值並不能互相比較。例如，一個很沒有接電路耐受性的人，a 會被設很高，讓演算法對於精力要素很敏感，但同時也會使所有數值膨脹，該膨脹後的數值大於另一個 a 很小的人的數值，完全並不代表前者比後者會 debug。
